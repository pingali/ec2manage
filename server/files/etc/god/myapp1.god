...
app_path="/mnt/www/myfavorite-app1/current"
app_num_instances = 5
app_port = 8000
...
if File.exists?(app_path) && File.directory?(app_path)
  ...
  app_port_range.each_with_index do |port, i|	
    ...
    God.watch do |w|		 
      ...
      w.group = 'myfavorite-app1'
      w.uid = @configs.user
      w.gid = @configs.group
      w.autostart = false

      w.start     = "mongrel_rails cluster::start    -C /etc/mongrel_cluster/myfavorite-app1.yml --clean --only #{port}"
      w.stop      = "mongrel_rails cluster::stop    -C /etc/mongrel_cluster/myfavorite-app1.yml --clean --only #{port}"
      w.restart   = "sleep #{i*rolling_delay}; mongrel_rails cluster::restart -C /etc/mongrel_cluster/myfavorite-app1.yml --clean --only #{port}"
      
      w.pid_file  = "/mnt/www/myfavorite-app1/shared/log/mongrel.#{port}.pid"
      w.grace     = 60.seconds
